#!/usr/bin/env python3
"""Build ONE fresh encoder PAL4 candidate from frozen FP16 weights, not from FP8.
...
from pal4_contract import (SCHEMA, SETTINGS, PINS, FROZEN_WEIGHTS, FP8_MANIFEST,
...
def main() -> None:
    ...
    control["tool_hashes"] = {name: sha256(tool_dir / name) for name in (
        "build_pal4_encoder.py", "pal4_contract.py", "w8_runtime_identity.py",
    ...
    # Verify the public API BEFORE an expensive model load. No guessed FP4 dtype.
    ...
    recipe = {"kind": "encoder", "format": "pal4", "transport": "packed", "control": control,
    ...
    try:
        ...
        metadata = asset_metadata("encoder-pal4-packed-v3-trial")
        metadata.model_description += "; PAL4 g16 scalar FP16 LUT; encoder-only unqualified experiment"
        ...
        if snapshot(AIModelAsset.load(path).program, spec["entrypoint"]) != after:
            raise ValueError("Saved PAL4 graph audit changed")
        ...
        if args.aot:
            ...
            if len(bundles) != 1:
                raise ValueError("Expected exactly one immutable PAL4 AOT bundle")

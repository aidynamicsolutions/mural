"""PAL6 encoder experiment contracts. No Apple or Torch imports at module load.

The v3 packet is unchanged except for a new format/name and a 56-value response.
...
SETTINGS = {
    "n_bits": 6, "method": "kmeans", "granularity": "per_grouped_channel",
...
def identity(recipe: dict[str, Any]) -> dict[str, Any]:
    if (recipe.get("kind"), recipe.get("format"), recipe.get("transport")) != (
            "encoder", "pal6", "packed"):
        raise ValueError("Only an encoder PAL6 packed-v3 experiment is supported")
...
def challenge(seed: int) -> list[int]:
    ...
    return [(seed + i * 13) % 32 for i in range(56)]
...
def coverage(before: dict[str, Any], after: dict[str, Any],
    ...
    if len(palettes) != len(weight_candidates):
        raise ValueError("PAL6 palette count does not match direct matrix weight constants")
    if after["operations"].get("coreai.blockwise_shift_scale", 0):
        raise ValueError("Unexpected scale/LUT quantization in scalar PAL6 recipe")
    ...
    for source, row in zip(weight_candidates, palettes):
        ...
        if (row["shape"] != source["shape"] or indices["shape"] != source["shape"]
                or row["dtype"] != "f16" or indices["dtype"] != "ui6"
                or indices["elements"] != source["elements"] or lut["dtype"] != "f16"
                or lut["shape"][-2:] != [64, 1]
                or lut["elements"] // 64 not in possible_groups
                or lut["elements"] % 64):
            raise ValueError("Ambiguous/unmatched PAL6 source weight mapping")
        ...
        compressed.append({**row, "source_key": source["key"],
                           ...
                           "logical_index_bytes": (indices["elements"] * 6 + 7) // 8,
    ...
    if skipped != set(remaining):
        raise ValueError("Source constants disappeared without a mapped PAL6 replacement")
...
def validate_pair(reference: dict[str, Any], candidate: dict[str, Any], *, rehash: bool = True) -> None:
    ...
    if (candidate.get("schema") != SCHEMA or candidate.get("status") != "aot-static-only"
        ...
        raise ValueError("PAL6 manifest contract/coverage is incomplete")
    ...
    if recomputed != candidate["coverage"]:
        raise ValueError("PAL6 coverage summary does not match tensor evidence")
    ...
    for field in ("source", "aot"):
        ...
        if rehash and bundle_record(Path(artifact["path"])) != artifact:
            raise ValueError(f"Changed PAL6 {field} artifact")
        rows = artifact["native_hashes"]
        if len(rows) != 1:
            raise ValueError("Ambiguous PAL6 compiler identity")
        row = rows[0]
        ...
        if (not key or len(key) % 2 or any(c not in "0123456789abcdef" for c in key)
            ...
            raise ValueError(f"Unsafe/colliding PAL6 {field} identity")
